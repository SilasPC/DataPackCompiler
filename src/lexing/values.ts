...
export const keywords: ReadonlyArray<string> = ['pub','extends','struct','recipe','ref','mod','fn','let','var','break','for','event','on','while','return','if','else','class','use','const']
export type Keyword = 'pub' | 'extends' | 'struct' | 'recipe' | 'ref' | 'mod' | 'fn' | 'let' | 'var' | 'break' | 'for' | 'event' | 'on' | 'while' | 'return' | 'if' | 'else' | 'class' | 'use' | 'const'
