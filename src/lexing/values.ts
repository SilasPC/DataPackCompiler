...
export const keywords: ReadonlyArray<string> = ['recipe','ref','namespace','fn','let','var','break','for','event','while','return','if','else','class','tick','import','const','from','export']
export type Keyword = 'recipe' | 'ref' | 'namespace' | 'fn' | 'let' | 'var' | 'break' | 'for' | 'event' | 'while' | 'return' | 'if' | 'else' | 'class' | 'tick' | 'import' | 'const' | 'from' | 'export'
