...
export const keywords: ReadonlyArray<string> = ['implements','struct','recipe','ref','mod','fn','let','var','break','for','event','while','return','if','else','class','tick','import','const','from','export']
export type Keyword = 'implements' | 'struct' | 'recipe' | 'ref' | 'mod' | 'fn' | 'let' | 'var' | 'break' | 'for' | 'event' | 'while' | 'return' | 'if' | 'else' | 'class' | 'tick' | 'import' | 'const' | 'from' | 'export'
