...
export const keywords: ReadonlyArray<string> = ['implements','struct','recipe','ref','mod','fn','let','var','break','for','event','on','while','return','if','else','class','use','const']
export type Keyword = 'implements' | 'struct' | 'recipe' | 'ref' | 'mod' | 'fn' | 'let' | 'var' | 'break' | 'for' | 'event' | 'on' | 'while' | 'return' | 'if' | 'else' | 'class' | 'use' | 'const'
