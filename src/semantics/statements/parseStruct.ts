...
export function parseStruct(node:ASTStructNode,scope:Scope,ctx:CompileContext): Maybe<StructDeclaration> {
    ...
    return maybe.map<Struct,StructDeclaration>(Struct.create(node.identifier.value,parents),struct=>({type:DeclarationType.STRUCT,struct,namePath:scope.getScopeNames()}))
}